  ...
  return (
    <div className="min-h-screen bg-[#f3f9ff] pt-24 pb-16 px-[2%] lg:px-[12%] sm:px-[8%]">
      <div className="mb-6">
        ...
        <p className="text-[#576070] mt-1">Manage your student account information.</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl border border-[#ebecef] p-8 text-center shadow-sm h-fit">
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-[#076dcd] to-[#18a54a] flex items-center justify-center text-white text-4xl font-bold mx-auto mb-4">
            {user?.fullName?.[0]?.toUpperCase()}
          </div>
          <h2 className="font-bold text-[#222e48] text-xl">{user?.fullName}</h2>
          <p className="text-[#576070] text-sm mt-1">{user?.email}</p>
          <span className="inline-block mt-3 bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xs font-semibold">Student</span>
        </div>
          ...
          <form onSubmit={handleSave} className="space-y-5">
            <div>
              ...
              <input value={form.fullName} onChange={e => setForm({...form, fullName: e.target.value})}
                className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#076dcd]" />
            </div>
            <div>
            ...
            <button type="submit" disabled={saving}
              className="bg-[#076dcd] hover:bg-black text-white px-6 py-2.5 rounded-full font-medium text-sm transition-colors cursor-pointer disabled:opacity-50">
